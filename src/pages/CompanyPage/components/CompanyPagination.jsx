...
const PageBtn = ({ active, disabled, onClick, children }) => (
    <button
        ...
        disabled={disabled}
        className={`
            flex items-center justify-center w-8 h-8 text-[13px] rounded-md border transition-colors
            ${active
...
const CompanyPagination = ({ page, pageSize, total, onPageChange, onPageSizeChange }) => {
    ...
    return (
        // flex-col trên mobile, flex-row từ sm trở lên
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                ...
                {/* Ẩn text "Showing..." trên màn rất nhỏ nếu cần, nhưng ở đây vẫn show vì đã wrap */}
                <span className="text-[13px] text-gray-500 whitespace-nowrap">
                    Showing {from}–{to} of {total}
                </span>
            </div>

            {/* Right: page buttons — scroll ngang nếu nhiều trang */}
            <div className="flex items-center gap-1 flex-wrap">
                ...
                {pageList.map((p, idx) =>
                    p === '...' ? (
                        <span key={`ellipsis-${idx}`} className="w-8 text-center text-[13px] text-gray-400">
                            ...
                        </span>
                    ) : (
