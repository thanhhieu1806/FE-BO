...
const PageBtn = ({ active, disabled, onClick, children }) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`flex items-center justify-center w-8 h-8 text-[13px] rounded-md border transition-colors ${active ? 'bg-blue-600 border-blue-600 text-white font-semibold' : disabled ? 'border-[#E5E7EB] text-gray-300 bg-white cursor-not-allowed' : 'border-[#E5E7EB] text-gray-600 bg-white hover:bg-gray-50'}`}
    >
        {children}
    </button>
);
...
const ConnectorPagination = ({ page, pageSize, total, onPageChange, onPageSizeChange }) => {
    ...
    return (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-1">
                <PageBtn disabled={page <= 1} onClick={() => onPageChange(page - 1)}><ChevronLeft /></PageBtn>
                {pageList.map((p, idx) =>
                    p === '...' ? (
                ...
                <PageBtn disabled={page >= totalPages} onClick={() => onPageChange(page + 1)}><ChevronRight /></PageBtn>
            </div>
        </div>
    );
};

export default ConnectorPagination;